...
void main() {
  ...
  //
  // Regression: GlassMenu was subscribing to the full MediaQueryData via
  // MediaQuery.of(context) in didChangeDependencies. Because MediaQueryData
  // includes viewInsets (keyboard height), opening/closing the soft keyboard
  // caused _GlassMenuState.didChangeDependencies to fire and triggered a full
  // rebuild of every GlassMenu in the tree — even ones that were closed.
  //
  // Fix: use MediaQuery.disableAnimationsOf(context) which subscribes only to
  // the disableAnimations field and ignores all other MediaQuery changes.
  testWidgets(
      'GlassMenu does NOT rebuild when viewInsets changes (keyboard open/close)',
      (tester) async {
    // Build counter — incremented every time GlassMenu's subtree builds.
    int buildCount = 0;
        ...
        home: Scaffold(
          body: Center(
            child: GlassMenu(
              ...
              items: [
                GlassMenuItem(title: 'CountItem', onTap: () {}),
              ],
            ),
          ),
        ),
    ...
    // Record the build count after initial layout (typically 1–2 builds).
    await tester.pump();
    final int buildCountAfterInit = buildCount;

    // Simulate a keyboard appearing: push viewInsets.bottom up (as the OS
    // does when the soft keyboard slides in). This is the exact MediaQuery
    // field that was causing spurious rebuilds before the fix.
    tester.view.viewInsets = FakeViewPadding(bottom: 300);
    await tester.pump();

    // The GlassMenu trigger must NOT have rebuilt.
    expect(
      buildCount,
      buildCountAfterInit,
      reason: 'GlassMenu rebuilt when viewInsets changed (keyboard opened). '
          'This means didChangeDependencies is subscribing to the full '
          'MediaQueryData instead of MediaQuery.disableAnimationsOf().',
    );

    // Simulate keyboard closing: viewInsets back to zero.
    ...
    expect(
      buildCount,
      buildCountAfterInit,
      reason: 'GlassMenu rebuilt when viewInsets changed (keyboard closed). '
          'Same root cause as above.',
    );

    // Reset view so teardown is clean.
