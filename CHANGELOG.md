...
- **Theme-level PlatformView fallback colour** — `GlassThemeSettings` now exposes and preserves `platformViewFallbackColor`, allowing premium glass surfaces to inherit one shared fallback for uncapturable PlatformView pixels instead of requiring per-widget settings.
